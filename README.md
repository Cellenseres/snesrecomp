...
| Nintendo DSP-1 / DSP-1B | **Implemented; SMK validation pending** | Instruction-level NEC uPD7725 core and Super Mario Kart's canonical SHVC-1K1X windows are implemented. A title-gated Fast HiROM mapping supports converted SMK development derivatives without broadening detection to other NEC DSP boards. Requires a user-supplied `dsp1b.rom`, `dsp1.rom`, or word-reversed `dsp1.bin`; no firmware is redistributed. |
