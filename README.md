...
| Nintendo DSP-1 / DSP-1B | **Implemented; clean SMK qualification pending** | Super Mario Kart's canonical SHVC-1K1X windows, an instruction-level NEC uPD7725 core, and a firmware-free command model for SMK's observed attract-mode command set are implemented. Supplied `dsp1b.rom`, `dsp1.rom`, and word-reversed `dsp1.bin` images always select LLE; when none is found, HLE is used and stops loudly on an unverified command. A 36,000-frame native development soak covers commands `00`, `02`, `04`, `06`, `0a`, `0c`, `28`, and `80` on both backends with bounded command and host-traffic drift; strict qualification still requires the canonical clean ROM. A title-gated Fast HiROM mapping supports converted SMK development derivatives without broadening detection to other NEC DSP boards. |
